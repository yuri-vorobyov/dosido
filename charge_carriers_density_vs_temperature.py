...
if __name__ == '__main__':
    tdp = TemperatureDependentParams(300, 3.9e21, 3.9e21, 0.68, 0.20, 0.95, 0.14, 209.5, 1.49, 9.13)
    vbt = BandTail(AllowedBand.VALENCE, 3.85e21, 30e-3, tdp.Eg, 5.0e-10, 5.0e-10)
    cbt = BandTail(AllowedBand.CONDUCTION, 3.85e21, 73-3, tdp.Eg, 5.0e-12, 5.0e-11)
    acceptor = GaussianDefect(ChargedState.ACCEPTOR, 2.8e20, 24e-3, 0.68, 3.0e-11, 1.5e-12)
    donor = GaussianDefect(ChargedState.DONOR, 2.8e20, 24e-3, 0.20, 2.5e-12, 5.0e-11)
    ...
    temperature = np.linspace(200, 350, 50)
